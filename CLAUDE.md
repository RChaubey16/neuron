...
- **A Prisma query builder call (e.g. `prisma.apiKey.update(...)`) returns a lazy "PrismaPromise" that only executes once something calls `.then()`/`.catch()` on it (or awaits it)** — unlike a native `Promise`, whose executor runs eagerly regardless of subscription. `void prisma.apiKey.update(...)` for a fire-and-forget write (e.g. `ApiKeyGuard`'s non-blocking `lastUsedAt` update) silently never sends the query: no error, no log, the row just never changes. The fix is `.catch(() => {})` (or `.then()`), not `void` — this actually subscribes, triggering execution while still not blocking the caller. Confirmed by reproducing the silent-no-op against the real Supabase DB, and by a guard unit test that mocks a lazy thenable (a real `Promise` mock can't catch this, since it'd execute regardless of subscription).
