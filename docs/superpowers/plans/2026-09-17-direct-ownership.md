...
-- Step 3: null out apiKeyId for rows that were only ever created through a
-- hidden dashboard system key -- they were honestly dashboard-originated
-- with no real key, which is what apiKeyId should say now
...
-- Step 4: userId is now populated on every row -- make it required
...
-- Step 5: drop the hidden system keys now that nothing references them
-- (every row that did has had apiKeyId nulled out above)
DELETE FROM "ApiKey" WHERE "isSystemKey" = true;

-- Step 6: drop the system-key column and its partial unique index
DROP INDEX "ApiKey_userId_system_key_unique";
ALTER TABLE "ApiKey" DROP COLUMN "isSystemKey";

-- Step 7: apiKeyId becomes optional metadata rather than the row's ownership
-- backbone -- drop and re-add its FK as ON DELETE SET NULL (was CASCADE) so
-- a future hard-deleted key can't take usage history down with it; userId
-- (CASCADE, matching ApiKey's own FK to User) is the ownership column now
...
-- Step 8: index the new ownership column, matching every other FK on these tables
